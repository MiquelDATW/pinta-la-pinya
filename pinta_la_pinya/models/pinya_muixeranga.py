...
class PinyaMuixeranga(models.Model):
    ...
    m_line_ids = fields.One2many('pinya.muixeranga.line', 'muixeranga_id', string="Línees", copy=True)
    ...
    def _compute_lliures(self):
        membres_actuacio = self.actuacio_id.membre_actuacio_ids.mapped('employee_id')
        membres = self.m_line_ids.mapped('membre_id')
        altres = self.mestra_id + self.passadora_id + self.estiradora_id
        xicalleres = self.resp_xicalla_ids
        lliures = membres_actuacio - membres - altres - xicalleres
    ...
    def _compute_cordons(self):
        ...
        for muix in muixs:
            pinyes = muix.filtered(lambda x: x.m_line_ids.tipus == 'pinya')
            c = max(pinyes.mapped('cordo') or [0])
            muix.cordons = c
    ...
    def _compute_total_count(self):
        for muix in self:
            pinyes = muix.m_line_ids.filtered(lambda x: x.tipus == 'pinya')
            troncs = muix.m_line_ids.filtered(lambda x: x.tipus == 'tronc')
            t = len(pinyes.ids)
            p = len(troncs.ids)
            muix.tronc_count = t
            muix.pinya_count = p
    ...
    def action_cancel(self):
        ...
        if bool(action_cancel) and actuacio_state == 'done':
            troncs = self.m_line_ids.filtered(lambda x: x.tipus == 'tronc')
            for tronc in troncs:
                tronc.membre_level_id = False
            pinyes = self.m_line_ids.filtered(lambda x: x.tipus == 'pinya')
            for pinya in pinyes:
                pinya.membre_level_id = False
        self.state = 'cancel'
    ...
    def tronc_muixeranga(self):
        view_search_id = self.env.ref('pinta_la_pinya.view_muixeranga_tronc_search').id
        ...
        name = self.name
        domain = [('id', 'in', self.m_line_ids.filtered(lambda x: x.tipus == 'tronc').ids)]
        action = {
            ...
            'target': 'current',
            'res_model': 'pinya.muixeranga.line',
            'context': {},
            'domain': domain,
        }
        return action
    ...
    def pinya_muixeranga(self):
        view_search_id = self.env.ref('pinta_la_pinya.view_muixeranga_pinya_search').id
        ...
        name = self.name
        domain = [('id', 'in', self.m_line_ids.filtered(lambda x: x.tipus == 'pinya').ids)]
        action = {
            'type': 'ir.actions.act_window',
            'views': [(view_tree_id, 'tree'), (view_form_id, 'form')],
            'search_view_id': view_search_id,
...
class PinyaMuixerangaLine(models.Model):
    ...
    _description = "Linea de muixeranga"
    _order = "data desc, muixeranga_id, pis, cordo, rengle, posicio_id asc"
    ...
    active = fields.Boolean(string="Actiu", default=True)
    data = fields.Datetime(string="Data", related="muixeranga_id.actuacio_id.data_inici", readonly=True, store=True)
    ...
    membre_id = fields.Many2one(string="Membre", comodel_name="hr.employee",
                                      related="membre_level_id.employee_id", store=True)
    membre_level_id = fields.Many2one(string="Membre Level", comodel_name="hr.employee.level")
    ...
    actuacio_id = fields.Many2one(string="Actuació", related="muixeranga_id.actuacio_id", readonly=True, store=True)
    muixeranga_state = fields.Selection(string='Estat muixeranga', related='muixeranga_id.state', store=True)
    plantilla_id = fields.Many2one(string="Plantilla", related="muixeranga_id.plantilla_id", readonly=True, store=True)

    recomanats_ids = fields.Many2many(string="Recomanats", comodel_name="hr.employee.level", compute="_compute_recomanats")

    _sql_constraints = [
        ('muixeranga_membre_uniq', 'unique(muixeranga_id, membre_id)',
         "Aquest membre ja forma part de la figura❗"),
    ]

    @api.multi
    @api.depends('membre_id', 'actuacio_id')
    def _compute_employee_actuacio(self):
        emp_act_obj = self.env['hr.employee.actuacio']
        m_lines = self.filtered(lambda x: x.membre_id and x.actuacio_id)
        for m_line in m_lines:
            membre = m_line.membre_id.id
            actuacio = m_line.actuacio_id.id
            membre_actuacio = emp_act_obj.search([('employee_id', '=', membre), ('actuacio_id', '=', actuacio)])
            m_line.employee_actuacio_id = membre_actuacio.id

    @api.multi
    def _compute_recomanats(self):
        if not bool(self.ids):
            return False
        muixeranga = self.mapped('muixeranga_id')
        if len(muixeranga) > 1:
            return False
        lliures = muixeranga.lliure_ids
        m_lines = muixeranga.m_line_ids
        for m_line in m_lines:
            levels = m_line.posicio_id.employee_level_ids
            mu1 = lliures.filtered(lambda x: m_line.posicio_id.id in x.posicio_ids.ids)
            recomanats = levels.filtered(lambda x: x.employee_id.id in mu1.ids).sorted('level', reverse=True)
            m_line.recomanats_ids = [(6, 0, recomanats.ids)]

    @api.onchange('posicio_id', 'cordo')
    def _onchange_posicio_cordo(self):
        if self.tipus == 'tronc':
            return False
    ...
    def _onchange_posicio_pis(self):
        if self.tipus == 'pinya':
            return False
    ...
    def create(self, vals):
        res = super(PinyaMuixerangaLine, self).create(vals)
        return res

    @api.multi
    def write(self, vals):
        res = super(PinyaMuixerangaLine, self).write(vals)
        return res
